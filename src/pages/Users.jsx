...
const ROLE_CONFIG = {
  admin:    { label: 'Admin',    color: '#7B68EE', bg: 'rgba(123,104,238,0.1)' },
...
export default function Users() {
  ...
  const inputStyle = { background: '#F4F4FA', border: '1.5px solid #E8E8F0', color: '#1A1A2E' }

  return (
    <div className="flex flex-col h-full overflow-y-auto">
      <div className="px-6 pt-6 pb-4">
        <h1 className="text-lg font-bold" style={{ color: '#1A1A2E' }}>Usuarios</h1>
        <p className="text-xs mt-0.5" style={{ color: '#9090B0' }}>Gestiona roles y perfiles de facturación</p>
      </div>
      ...
      <div className="px-6 pb-6">
        <div className="rounded-2xl overflow-hidden" style={{ background: '#fff', border: '1.5px solid #E8E8F0', boxShadow: '0 1px 8px rgba(0,0,0,0.04)' }}>
          {/* Table header */}
          <div className="grid grid-cols-12 gap-4 px-4 py-3 text-xs font-bold uppercase tracking-wider" style={{ borderBottom: '1px solid #F0F0F8', color: '#9090B0' }}>
              ...
              >
                ...
                <div className="col-span-4 flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-semibold truncate" style={{ color: '#1A1A2E' }}>{m.profiles?.full_name || 'Sin nombre'}</p>
                    <p className="text-xs truncate" style={{ color: '#9090B0' }}>{m.profiles?.email || ''}</p>
                  </div>
                </div>
                ...
                <div className="col-span-3">
                  {isEditingThis ? (
                    ...
                    <p className="text-sm truncate" style={{ color: '#6B6B8A' }}>{m.profiles?.job_title || '—'}</p>
                  )}
                </div>
                ...
                <div className="col-span-2 text-right">
                  {isEditingThis ? (
                    ...
                    <span className="font-numeric text-sm font-semibold" style={{ color: m.profiles?.hourly_rate ? '#1A1A2E' : '#C0C0D8' }}>
                ...
                <div className="col-span-1 flex items-center justify-end gap-1">
                  {isEditingThis ? (
                    ...
                    <button onClick={() => startEdit(m)}
                      ...
                      style={{ color: '#B0B0C8' }}
                      onMouseEnter={e => { e.currentTarget.style.background = 'rgba(123,104,238,0.1)'; e.currentTarget.style.color = '#7B68EE'; e.currentTarget.style.opacity = '1' }}
